...
st.set_page_config(
    page_title="टपाल व्यवस्थापन व कार्यविवरण नोंदवही",
    layout="wide",
    page_icon="🏛️",
)
...
# ----------------- FIXED DUAL MIC VOICE TYPING COMPONENT -----------------
def voice_typing_component(element_id):
  safe_id = "".join(filter(str.isalnum, element_id))
  component_html = f"""
    <div style="margin-bottom: 5px; display: flex; gap: 5px;">
        <button id="btn_mr_{safe_id}" style="background-color:#2a5298; color:white; border:none; padding:4px 8px; border-radius:4px; cursor:pointer; font-size:11px;">🎤 मराठी</button>
        <button id="btn_en_{safe_id}" style="background-color:#1e3c72; color:white; border:none; padding:4px 8px; border-radius:4px; cursor:pointer; font-size:11px;">🎤 इंग्रजी</button>
        <span id="status_{safe_id}" style="font-size:10px; color: gray; align-self: center;"></span>
        
        <script>
            const safeId = "{safe_id}";
            const labelText = "{element_id}";
            
            function recognizeSpeech(langCode) {{
                if (!('webkitSpeechRecognition' in window) && !('SpeechRecognition' in window)) {{
                    alert("तुमचा ब्राऊजर व्हॉइस रिकग्निशन सपोर्ट करत नाही. कृपया गुगल क्रोम (Google Chrome) वापरा.");
                    return;
                }}
                ...
                const statusSpan = window.parent.document.getElementById('status_' + safeId);
                if(statusSpan) statusSpan.innerText = "ऐकत आहे...";

                recognition.onresult = function(event) {{
                    const transcript = event.results[0][0].transcript;
                    const doc = window.parent.document;
                    
                    const textContainers = doc.querySelectorAll('.stTextInput, .stTextArea');
                    let targetInput = null;
                    
                    textContainers.forEach(container => {{
                        const label = container.querySelector('label');
                        if (label && label.innerText.includes(labelText)) {{
                            targetInput = container.querySelector('input, textarea');
                        }}
                    }});
                    ...
                    if (targetInput) {{
                        ...
                        let newVal = targetInput.value ? targetInput.value + ' ' + transcript : transcript;
                        
                        if (nativeInputValueSetter) {{
                            nativeInputValueSetter.call(targetInput, newVal);
                        }} else {{
                            targetInput.value = newVal;
                        }}
                        
                        targetInput.dispatchEvent(new window.parent.Event('input', {{ bubbles: true }}));
                        targetInput.dispatchEvent(new window.parent.Event('change', {{ bubbles: true }}));
                    }}
                ...
                recognition.onerror = function(event) {{
                    if(statusSpan) statusSpan.innerText = "त्रुटी आली";
                }};
            ...
            setTimeout(() => {{
                const btnMr = document.getElementById('btn_mr_' + safeId);
                const btnEn = document.getElementById('btn_en_' + safeId);
                
                if(btnMr) {{ btnMr.onclick = () => recognizeSpeech('mr-IN'); }}
                if(btnEn) {{ btnEn.onclick = () => recognizeSpeech('en-US'); }}
            }}, 500);
        </script>
    </div>
    """
...
def format_to_ddmmyyyy(date_val):
  if (
      pd.isna(date_val)
      or not date_val
      or str(date_val).lower() in ["nan", "nat", "none", ""]
  ):
    return ""
...
def calculate_pending_days(inward_date_str):
  if (
      not inward_date_str
      or pd.isna(inward_date_str)
      or str(inward_date_str).lower() in ["nan", "nat", "none", ""]
  ):
    return ""
  try:
    inward_dt = pd.to_datetime(
        inward_date_str, format="%d/%m/%Y", dayfirst=True, errors="coerce"
    )
    if pd.isna(inward_dt):
      inward_dt = pd.to_datetime(
          inward_date_str, dayfirst=True, errors="coerce"
      )
    if pd.isna(inward_dt):
      return ""
...
def clean_df(df):
  if df is not None and not df.empty:
    df = df.fillna("")
    df = df.replace(["NaN", "nan", "NaT", "NAT", "None", "NONE", "none"], "")
  ...
  for _, row in df.iterrows():
    ...
    for val in row:
      text = (
          ...
          if pd.notna(val)
          and str(val).strip() != ""
          and str(val).lower() not in ["none", "nan", "nat"]
          else ""
      )
...
<div class="main-header">
    ...
    <p>टपाल नोंद, पुढील कार्यवाही अद्ययावत करणे, गोषवारा व रिपोर्ट्स (Dual Mic Voice Typing)</p>
</div>
""",
    unsafe_allow_html=True,
)
...
with tab1:
  ...
  with st.form("tapal_form", clear_on_submit=True):
    ...
    with col1:
      ...
      voice_typing_component("File No")

      subject_code = st.text_input("Subject Code")
      voice_typing_component("Subject Code")

      inward_no = st.text_input("Inward No")
      voice_typing_component("Inward No")

      computer_no = st.text_input("Computer No")
      voice_typing_component("Computer No")

      inward_date_val = st.date_input(
          "Inward Date (प्राप्त दिनांक)", datetime.now()
      )
      inward_date = inward_date_val.strftime("%d/%m/%Y")

      letter_no_date = st.text_input("Letter No & Date")
      voice_typing_component("Letter No & Date")

      letter_from = st.text_input("From (कोणाकडून मिळाले / अर्जदार)")
      voice_typing_component("From (कोणाकडून मिळाले / अर्जदार)")
    ...
    with col2:
      ...
      voice_typing_component("Subject (विषय)")

      emp_name = st.text_input("Emp Name / तक्रारदार")
      voice_typing_component("Emp Name / तक्रारदार")

      address = st.text_input("Address")
      voice_typing_component("Address")

      district = st.selectbox(
          "District", ["नाशिक", "धुळे", "जळगाव", "नंदुरबार", "अहिल्यानगर", "इतर"]
      )

    with col3:
      computer_no_2 = st.text_input("Computer No 2 (इतर संदर्भ)")
      voice_typing_component("Computer No 2 (इतर संदर्भ)")

      remarks = st.text_area("शेरा (Remarks)")
      voice_typing_component("शेरा (Remarks)")
...
with tab2:
  ...
  df_all_tapal = pd.read_sql_query(
      "SELECT id, inward_no, subject, inward_date, action_taken FROM"
      " tapal_entries ORDER BY id DESC",
      conn,
  )
  ...
  if not df_all_tapal.empty:
    tapal_options = {
        f"ID {row['id']} | आवक क्र:"
        f" {get_clean_number(row['inward_no']) or 'N/A'} | विषय:"
        f" {str(row['subject'])[:30]}...": row["id"]
        for _, row in df_all_tapal.iterrows()
    }

    selected_option = st.selectbox(
        "🎯 कार्यवाही अपडेट करण्यासाठी प्रकरण निवडा:",
        list(tapal_options.keys()),
    )
    selected_id = tapal_options[selected_option]

    entry = pd.read_sql_query(
        "SELECT * FROM tapal_entries WHERE id = ?", conn, params=(selected_id,)
    ).iloc[0]
    ...
    col_info1.write(
        f"**आवक क्रमांक:** {get_clean_number(entry['inward_no']) or '-'}"
    )
    col_info1.write(
        f"**आवक दिनांक:** {format_to_ddmmyyyy(entry['inward_date']) or '-'}"
    )
    col_info2.write(
        f"**अर्जदार/प्राधिकरण:** {entry['emp_name'] or entry['letter_from'] or '-'}"
    )
    ...
    with st.form("update_action_form"):
      ...
      with col_u1:
        ...
        u_act_date_val = st.date_input(
            "Action Taken Date (कार्यवाही दिनांक):", value=existing_act_date
        )
        u_action_taken_date = (
            u_act_date_val.strftime("%d/%m/%Y") if u_act_date_val else ""
        )

        raw_act_taken = (
            entry["action_taken"]
            if pd.notna(entry["action_taken"])
            and str(entry["action_taken"]).lower() not in ["nan", "nat", "none"]
            else ""
        )
        u_action_taken = st.text_input(
            "केलेली कार्यवाही (Action Taken - भरल्यास प्रकरण निपटारा होईल):",
            value=raw_act_taken,
        )
        voice_typing_component(
            "केलेली कार्यवाही (Action Taken - भरल्यास प्रकरण निपटारा होईल)"
        )

        raw_info_req = (
            entry["info_requested_from"]
            if pd.notna(entry["info_requested_from"])
            and str(entry["info_requested_from"]).lower()
            not in ["nan", "nat", "none"]
            else ""
        )
        u_info_requested_from = st.text_input(
            "कोणाकडून माहिती मागविली:", value=raw_info_req
        )
        voice_typing_component("कोणाकडून माहिती मागविली")

        raw_final_act = (
            entry["final_action"]
            if pd.notna(entry["final_action"])
            and str(entry["final_action"]).lower() not in ["nan", "nat", "none"]
            else ""
        )
        u_final_action = st.text_input(
            "अंतिम कार्यवाही प्रकार (Final Action):", value=raw_final_act
        )
        voice_typing_component("अंतिम कार्यवाही प्रकार (Final Action)")

      with col_u2:
        raw_off_let = (
            entry["office_letter_no_date"]
            if pd.notna(entry["office_letter_no_date"])
            and str(entry["office_letter_no_date"]).lower()
            not in ["nan", "nat", "none"]
            else ""
        )
        u_office_letter_no_date = st.text_input(
            "कार्यालयीन पत्र क्र. व दिनांक:", value=raw_off_let
        )
        voice_typing_component("कार्यालयीन पत्र क्र. व दिनांक")

        existing_rem_date = parse_date_safely(entry["reminder_letter_date"])
        u_rem_date_val = st.date_input(
            "तगादा केला असेल तर पत्र दिनांक:", value=existing_rem_date
        )
        u_reminder_letter_date = (
            u_rem_date_val.strftime("%d/%m/%Y") if u_rem_date_val else ""
        )

        raw_let_sent = (
            entry["letter_sent_to"]
            if pd.notna(entry["letter_sent_to"])
            and str(entry["letter_sent_to"]).lower()
            not in ["nan", "nat", "none"]
            else ""
        )
        u_letter_sent_to = st.text_input(
            "पत्र कोणाला पाठवले (Sent To):", value=raw_let_sent
        )
        voice_typing_component("पत्र कोणाला पाठवले (Sent To)")

        raw_close_date = (
            entry["application_close_date"]
            if pd.notna(entry["application_close_date"])
            and str(entry["application_close_date"]).lower()
            not in ["nan", "nat", "none"]
            else ""
        )
        u_application_close_date = st.text_input(
            "प्रकरण बंद दिनांक (Close Date):", value=raw_close_date
        )
        voice_typing_component("प्रकरण बंद दिनांक (Close Date)")

      with col_u3:
        paper_rec_bool = (
            True if str(entry["paper_go_in_record"]).strip() == "होय" else False
        )
        u_paper_rec = st.checkbox(
            "Paper Go in Record (अभिलेखात जमा)", value=paper_rec_bool
        )

        call_rep_bool = (
            True if str(entry["calling_report"]).strip() == "होय" else False
        )
        u_call_rep = st.checkbox(
            "Calling Report (अहवाल मागविला)", value=call_rep_bool
        )

        raw_final_det = (
            entry["final_action_details"]
            if pd.notna(entry["final_action_details"])
            and str(entry["final_action_details"]).lower()
            not in ["nan", "nat", "none"]
            else ""
        )
        u_final_action_details = st.text_area(
            "अंतिम सविस्तर शेरा (Final Details):", value=raw_final_det
        )
        voice_typing_component("अंतिम सविस्तर शेरा (Final Details)")

        raw_remarks = (
            entry["remarks"]
            if pd.notna(entry["remarks"])
            and str(entry["remarks"]).lower() not in ["nan", "nat", "none"]
            else ""
        )
        u_remarks = st.text_area("सामान्य शेरा (Remarks):", value=raw_remarks)
        voice_typing_component("सामान्य शेरा (Remarks)")

      update_btn = st.form_submit_button(
          "💾 अद्ययावत करा (Save Changes)", type="primary"
      )
      ...
      if update_btn:
        ...
        c.execute(
            ...
            (
                u_action_taken_date,
                u_action_taken,
                u_info_requested_from,
                u_office_letter_no_date,
                u_reminder_letter_date,
                u_final_action,
                u_final_action_details,
                u_letter_sent_to,
                u_application_close_date,
                paper_go_val,
                calling_rep_val,
                u_remarks,
                selected_id,
...
with tab3:
  ...
  if not df_all.empty:
    pending_count = len(
        df_all[df_all["action_taken"].astype(str).str.strip() == ""]
    )
    ...
    m6.metric(
        "न्यायालयीन संदर्भ", len(df_all[df_all["letter_type"] == "न्यायालयीन"])
    )
    ...
    with col_acc1:
      ...
      summary_type = (
          df_all.groupby("letter_type").size().reset_index(name="संख्या (Count)")
      )
  ...
  with col_ul:
    st.subheader("जुनी फाईल (Excel किंवा CSV) अपलोड करा")
    uploaded_file = st.file_uploader(
        "तुमची जुनी Excel किंवा CSV फाईल निवडा (Append करण्यासाठी)",
        type=["csv", "xlsx", "xls"],
    )
    ...
    if uploaded_file is not None:
      if st.button("🚀 अपलोड केलेली फाईल डेटाबेसमध्ये समाविष्ट (Append) करा"):
        try:
          ...
          # स्मार्ट दिनांक पार्सिंग (dayfirst=True मुळे तारीख/महिना उलटा होणार नाही)
          for col in df_upload.columns:
            if any(term in col.lower() for term in ["date", "दिनांक"]):
              df_upload[col] = pd.to_datetime(
                  df_upload[col], dayfirst=True, errors="coerce"
              ).dt.strftime("%d/%m/%Y")
              df_upload[col] = df_upload[col].fillna("")

          # डेटाबेस टेबलमधील मूळ कॉलम्स तपासून जुळवून घेणे
          cursor = conn.cursor()
          ...
          df_upload = df_upload[
              [col for col in db_columns if col in df_upload.columns]
          ]

          df_upload.to_sql("tapal_entries", conn, if_exists="append", index=False)
          st.success(
              "✅ फाईलमधील सर्व डेटा यशस्वीरीत्या डेटाबेसमध्ये जोडला गेला आहे!"
          )
          st.rerun()
  ...
  df = pd.read_sql_query(
      "SELECT * FROM tapal_entries ORDER BY id DESC", conn
  )
  df = clean_df(df)
...
with tab4:
  ...
  if not df_p.empty:
    ...
    st.markdown("---")
    if st.checkbox(
        "🖨️ प्रलंबित संदर्भ प्रिंट / PDF व्ह्यू दाखवा", key="print_pending"
    ):
...
with tab5:
  st.markdown(
      "### ⏳ प्रतिक्षाधिन प्रकरणांची नोंदवही (महसूल आस्थापना) संकलन-"
      " (आस्था-५)"
  )
  ...
  if not df_await.empty:
    ...
    df_await[
        "तक्रारदाराचे नाव / प्राधिकरणाचे नाव / प्राप्त दिनांक"
    ] = df_await.apply(
        lambda r: f"{r['letter_from'] or ''}, दि. {format_to_ddmmyyyy(r['inward_date'])}".strip(),
        axis=1,
    )

    df_await["कार्यालयीन पत्र क्र. व दिनांक"] = df_await.apply(
        lambda r: f"दि. {format_to_ddmmyyyy(r['action_taken_date'])} रोजी {r['action_taken'] or ''}".strip()
        if r["action_taken_date"] and r["action_taken"]
        else (
            f"दि. {format_to_ddmmyyyy(r['action_taken_date'])}"
            if r["action_taken_date"]
            else (r["action_taken"] or "")
        ),
        axis=1,
    )

    df_await_display = df_await.rename(
        columns={
            "subject": "विषय",
        ...
        "तक्रारदाराचे नाव / प्राधिकरणाचे नाव / प्राप्त दिनांक",
        "विषय",
        "कोणाकडून माहिती मागविली",
        "कार्यालयीन पत्र क्र. व दिनांक",
        "तगादा केला असेल तर पत्र दिनांक",
        "शेरा",
    ]]
    ...
    st.markdown("---")
    if st.checkbox(
        "🖨️ प्रतिक्षाधिन प्रकरणे प्रिंट / PDF व्ह्यू दाखवा", key="print_await"
    ):
      render_html_table(
          df_await_display,
          "प्रतिक्षाधिन प्रकरणांची नोंदवही (महसूल आस्थापना) संकलन- (आस्था-५)",
      )
  else:
...
with tab6:
  st.markdown(
      "### 🖨️ कार्यविवरण नोंदवही (महसूल आस्थापना) संकलन - (आस्था-५) Worksheet"
  )
  filter_type = st.selectbox(
      "प्रिंटसाठी संदर्भ प्रकार निवडा:",
      [
          "सर्व टपाल",
          "शासन",
          "मंत्री",
  ...
  if not df_ws_raw.empty:
    ...
    if "inward_date" in df_ws_raw.columns:
      df_ws_raw["inward_date"] = df_ws_raw["inward_date"].apply(
          format_to_ddmmyyyy
      )
    if "action_taken_date" in df_ws_raw.columns:
      df_ws_raw["action_taken_date"] = df_ws_raw["action_taken_date"].apply(
          format_to_ddmmyyyy
      )

    df_ws = df_ws_raw.rename(
        columns={
            "inward_no": "आवक क्रमांक",
            "inward_date": "आवक दिनांक",
            "letter_no_date": "पत्र क्र. व दिनांक",
            "letter_from": "कोणाकडून मिळाले",
            "letter_type": "पत्राचा प्रकार",
            "subject": "पत्राचा विषय",
            "action_taken_date": "कार्यवाही दिनांक",
            "action_taken": "केलेली कार्यवाही",
        }
    )[[
        "अ.क्र.",
        "आवक क्रमांक",
    ...
    if st.checkbox(
        "🖨️ कार्यविवरण नोंदवही प्रिंट / PDF व्ह्यू दाखवा", key="print_worksheet"
    ):
      render_html_table(
          df_ws, f"कार्यविवरण नोंदवही संकलन- (आस्था-५) - {filter_type}"
      )
...
with tab7:
  ...
  voice_typing_component("१. कामाचा / पत्राचा विषय:")

  previous_pdf = st.file_uploader(
      "𝟐. फाईलचा पूर्व इतिहास / जुन्या टिपण्या (PDF):", type=["pdf"]
  )
  new_letter_pdf = st.file_uploader(
      "३. नवीन आलेले पत्र (PDF):", type=["pdf"]
  )

  additional_remarks = st.text_area("४. अतिरिक्त निर्देश / शेरा (ऐच्छिक):")
  voice_typing_component("४. अतिरिक्त निर्देश / शेरा (ऐच्छिक):")
    ...
    if not ai_subject or not new_letter_pdf:
      ...
      with st.spinner("Gemini AI टिपणी तयार करत आहे..."):
        try:
          ...
          prompt_content = (
              f"तुम्ही एक वरिष्ठ शासकीय अधिकारी/लिपीक आहात. खालील माहितीच्या"
              f" आधारे शुद्ध, अधिकृत मराठी शासकीय भाषेत टिपणी (Note Sheet) आणि"
              f" उत्तराचा मसुदा तयार करा.\n\nविषय: {ai_subject}\nमागील"
              f" इतिहास/टिपणी: {old_history_text}\nनवीन आलेले पत्र:"
              f" {new_letter_text}\nअतिरिक्त शेरा: {additional_remarks}\n\nखालील"
              " फॉरमॅटमध्ये उत्तर द्या:\n---कार्यालयीन टिपणी---\n[सविस्तर"
              " टिपणी]\n\n---जावक पत्र मसुदा---\n[जावक पत्राचा मसुदा]"
          )
